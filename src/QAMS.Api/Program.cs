...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        if (!environment.IsEnvironment("Testing"))
        {
            ...
            // HACK: Sincronización inteligente sin pérdida de datos en Render
            try
            {
                app.Logger.LogInformation("=== INICIO: Sincronización inteligente de BD ===");
                
                // 1. Crear las tablas de catálogo que faltan
                string[] createTableScripts = {
                    @"CREATE TABLE IF NOT EXISTS test_plan_types (
                        ...
                        CONSTRAINT ""PK_test_plan_types"" PRIMARY KEY (""Id"")
                    );",
                    ...
                    @"CREATE TABLE IF NOT EXISTS test_strategies (
                        ...
                        ""UpdatedByUserId"" uuid,
                        ""IsActive"" boolean NOT NULL,
                        CONSTRAINT ""PK_test_strategies"" PRIMARY KEY (""Id"")
                    );",
                    @"CREATE TABLE IF NOT EXISTS test_plan_environments (
                        ""Id"" integer GENERATED BY DEFAULT AS IDENTITY,
                        ""Code"" character varying(50) NOT NULL,
                        ""Name"" character varying(100) NOT NULL,
                        ""Description"" text,
                        ...
                        ""UpdatedByUserId"" uuid,
                        ""IsActive"" boolean NOT NULL,
                        CONSTRAINT ""PK_test_plan_environments"" PRIMARY KEY (""Id"")
                    );",
                    @"CREATE TABLE IF NOT EXISTS test_plan_levels (
                        ""Id"" integer GENERATED BY DEFAULT AS IDENTITY,
                        ""Code"" character varying(50) NOT NULL,
                        ""Name"" character varying(100) NOT NULL,
                        ""Description"" text,
                        ...
                        ""UpdatedByUserId"" uuid,
                        ""IsActive"" boolean NOT NULL,
                        CONSTRAINT ""PK_test_plan_levels"" PRIMARY KEY (""Id"")
                    );",
                    @"CREATE TABLE IF NOT EXISTS test_design_techniques (
                        ""Id"" integer GENERATED BY DEFAULT AS IDENTITY,
                        ""Code"" character varying(50) NOT NULL,
                        ""Name"" character varying(100) NOT NULL,
                        ""Description"" text,
                    ...
                    @"CREATE TABLE IF NOT EXISTS defect_severities (
                        ""Id"" integer GENERATED BY DEFAULT AS IDENTITY,
                        ""Code"" character varying(50) NOT NULL,
                        ""Name"" character varying(100) NOT NULL,
                        ""Description"" text,
                        ...
                        ""UpdatedByUserId"" uuid,
                        ""IsActive"" boolean NOT NULL,
                        CONSTRAINT ""PK_defect_severities"" PRIMARY KEY (""Id"")
                    );"
                };

                foreach (var sql in createTableScripts)
                {
                    db.Database.ExecuteSqlRaw(sql);
                }

                // Asegurar las columnas BDD que causaron problemas recientemente
                string[] alterTableScripts = {
                    "ALTER TABLE test_cases ADD COLUMN is_bdd boolean NOT NULL DEFAULT FALSE;",
                    "ALTER TABLE test_cases ADD COLUMN bdd_scenario text;"
                };
                foreach (var sql in alterTableScripts)
                {
                    try { db.Database.ExecuteSqlRaw(sql); } catch { /* Ignore error if column already exists */ }
                }
                ...
                // 3. Insertar historial de migraciones pendientes como completadas
                var allPendingMigrations = db.Database.GetPendingMigrations().ToList();
                if (allPendingMigrations.Any())
                {
                    app.Logger.LogInformation("Insertando {Count} migraciones en el historial para sincronizar.", allPendingMigrations.Count);
                    foreach (var migration in allPendingMigrations)
                    {
                        db.Database.ExecuteSqlRaw(@"
                ...
                app.Logger.LogInformation("=== FIN: Sincronización inteligente completada exitosamente ===");
            }
            catch (Exception ex)
            {
                app.Logger.LogError(ex, "ERROR al sincronizar BD en Render. Detalles: {Msg}", ex.Message);
            }
