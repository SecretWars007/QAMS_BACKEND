...
using QAMS.Infrastructure.Persistence;
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        ...
        if (!environment.IsEnvironment("Testing"))
        {
            ...
            // HACK: Autocuración Inteligente de Base de Datos para Render
            try
            {
                app.Logger.LogInformation("=== INICIO: Autocuración Inteligente de BD ===");

                // 1. Crear tabla de historial de migraciones si no existe
                ...
                // 2. Obtener todas las migraciones (pendientes y aplicadas)
                var migrator = ((Microsoft.EntityFrameworkCore.Infrastructure.IInfrastructure<IServiceProvider>)db).Instance.GetRequiredService<Microsoft.EntityFrameworkCore.Migrations.IMigrator>();
                var allMigrations = db.Database.GetPendingMigrations().ToList();

                if (allMigrations.Any())
                {
                    app.Logger.LogInformation($"Se detectaron {allMigrations.Count} migraciones no registradas en el historial. Iniciando curación...");

                    foreach (var migration in allMigrations)
                    {
                        // Generar el script exacto de esta migración
                        var script = migrator.GenerateScript(fromMigration: null, toMigration: migration, options: Microsoft.EntityFrameworkCore.Migrations.MigrationsSqlGenerationOptions.Default);
                        
                        // Dividir el script por punto y coma (;) para ejecutar comando por comando
                        ...
                        foreach (var stmt in statements)
                        {
                            ...
                            if (trimmedStmt.StartsWith("BEGIN", StringComparison.OrdinalIgnoreCase) || 
                                trimmedStmt.StartsWith("COMMIT", StringComparison.OrdinalIgnoreCase)) 
                                continue;

                            try
                            {
                                // Ejecutar cada comando individualmente. Si ya existe, fallará y lo ignoramos en silencio.
                                db.Database.ExecuteSqlRaw(trimmedStmt);
                            }
                            catch
                            {
                                // Silenciamos los errores de "relación ya existe", "columna ya existe", etc.
                                // Esto es vital porque EnsureCreated() ya creó parte del esquema.
                            }
                        }

                        // Finalmente, registrar la migración como aplicada para que no se intente de nuevo
                        db.Database.ExecuteSqlRaw(@"
                ...
                app.Logger.LogInformation("=== FIN: Autocuración completada exitosamente ===");
            }
            catch (Exception ex)
            {
                app.Logger.LogError(ex, "ERROR Crítico durante la autocuración de BD: {Msg}", ex.Message);
            }
